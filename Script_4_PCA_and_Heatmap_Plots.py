...
5. Venn Diagram for metabolites between A. robustus and C. churrovis
...
"""
Create Venn Diagram for Detected Metabolites Between AR and CC
"""
# Determine the number of metabolites (not "unknown") detected (non-zero values in at least 2/4 replicates for a sample type) in AR and CC samples
ar_detected = data_knowns[(data_knowns[ar_col_names] > 0).sum(axis=1) >= 2][CMPD_COL_NAME].unique()
cc_detected = data_knowns[(data_knowns[cc_col_names] > 0).sum(axis=1) >= 2][CMPD_COL_NAME].unique()

# Determine shared metabolites and metabolites in neither AR or CC
ar_cc_detected = set(ar_detected).intersection(set(cc_detected))
ar_only_detected = set(ar_detected) - ar_cc_detected
cc_only_detected = set(cc_detected) - ar_cc_detected
neither_detected = set(data_knowns[CMPD_COL_NAME]) - (ar_only_detected.union(cc_only_detected).union(ar_cc_detected))

# Determine the number of unknown metabolites detected (non-zero values in at least 2/4 replicates for a sample type) in AR and CC samples. Unknown metabolites are those that contain 'Unknown' in the CMPD_COL_NAME column
data_unknowns = data[data[CMPD_COL_NAME].str.contains('Unknown')]
ar_unknown_detected = data_unknowns[(data_unknowns[ar_col_names] > 0).sum(axis=1) >= 2][CMPD_COL_NAME].unique()
cc_unknown_detected = data_unknowns[(data_unknowns[cc_col_names] > 0).sum(axis=1) >= 2][CMPD_COL_NAME].unique()

# Determine shared unknown metabolites and unknown metabolites in neither AR or CC
...
# Create figure for Venn diagram
fig, ax = plt.subplots(figsize=(10, 8))

# Create custom Venn diagram with fixed circle sizes to avoid visual dominance of intersection
# Define circle parameters
radius = 0.4
center_left = (-0.2, 0)
center_right = (0.2, 0)

# Create circles manually
circle_ar = plt.Circle(center_left, radius, color=COLORS['AR'], alpha=0.3, label='A. robustus')
circle_cc = plt.Circle(center_right, radius, color=COLORS['CC'], alpha=0.3, label='C. churrovis')

# Add circles to plot
ax.add_patch(circle_ar)
ax.add_patch(circle_cc)

# Add circle outlines
circle_ar_outline = plt.Circle(center_left, radius, fill=False, edgecolor='black', linewidth=2)
circle_cc_outline = plt.Circle(center_right, radius, fill=False, edgecolor='black', linewidth=2)
ax.add_patch(circle_ar_outline)
ax.add_patch(circle_cc_outline)

# Add custom labels for known metabolites (black text)
# AR only (left side of left circle)
ax.text(-0.4, 0.0, str(len(ar_only_detected)), 
        fontsize=30, fontweight='bold', ha='center', va='center', color='black')
# CC only (right side of right circle)
ax.text(0.4, 0.0, str(len(cc_only_detected)), 
        fontsize=30, fontweight='bold', ha='center', va='center', color='black')
# AR and CC intersection (center overlap)
ax.text(0.0, 0.0, str(len(ar_cc_detected)), 
        fontsize=30, fontweight='bold', ha='center', va='center', color='black')

# Add unknown metabolite numbers in gray underneath known metabolite numbers
# AR only unknowns
ax.text(-0.4, -0.15, str(len(ar_only_unknown_detected)), 
        fontsize=30, ha='center', va='center', color='dimgray')
# CC only unknowns
ax.text(0.4, -0.15, str(len(cc_only_unknown_detected)), 
        fontsize=30, ha='center', va='center', color='dimgray')
# AR and CC intersection unknowns
ax.text(0.0, -0.15, str(len(ar_cc_unknown_detected)), 
        fontsize=30, ha='center', va='center', color='dimgray')

# Add circle labels with italics for species names
ax.text(-0.3, 0.5, r'$\mathit{A. robustus}$', 
        fontsize=30, fontweight='bold', ha='center', va='center')
ax.text(0.3, 0.5, r'$\mathit{C. churrovis}$', 
        fontsize=30, fontweight='bold', ha='center', va='center')

# Add "Neither" category at the bottom
ax.text(0.0, -0.5, f'{len(neither_detected)}', 
        fontsize=30, fontweight='bold', ha='center', va='center', color='black')
ax.text(0.0, -0.6, f'{len(neither_unknown_detected)}', 
        fontsize=30, ha='center', va='center', color='dimgray')

# Remove axis and set tight limits to reduce whitespace
ax.set_xlim(-0.8, 0.8)
ax.set_ylim(-0.7, 0.7)
ax.axis('off')
ax.set_aspect('equal')
...
fig.savefig(pjoin(OUTPUT_FOLDER, 'venn_diagram_metabolites_ar_cc_batch_3.png'), 
            dpi=600, bbox_inches='tight')

# Export the metabolite lists to CSV files for reference
ar_only_df = pd.DataFrame({'Metabolite': list(ar_only_detected)})
ar_only_df.to_csv(pjoin(OUTPUT_FOLDER, 'ar_only_metabolites_batch_3.csv'), index=False)

cc_only_df = pd.DataFrame({'Metabolite': list(cc_only_detected)})
cc_only_df.to_csv(pjoin(OUTPUT_FOLDER, 'cc_only_metabolites_batch_3.csv'), index=False)

shared_df = pd.DataFrame({'Metabolite': list(ar_cc_detected)})
shared_df.to_csv(pjoin(OUTPUT_FOLDER, 'shared_metabolites_ar_cc_batch_3.csv'), index=False)